...
class GitFixtureTests(unittest.TestCase):
    ...
    def _container_rewrite_arguments(self) -> tuple[str, ...]:
        ...
        return (
            ...
            "--output",
            str(self.fixture.output_dir / "output.git"),
            "--policy",
            str(policy),
        )
    ...
    def test_container_runner_translates_fixture_paths_and_uses_allowlisted_environment(self) -> None:
        policy = self.fixture.write_policy()
        output = self.fixture.output_dir / "output.git"
        arguments = (
            "rewrite",
            "--source",
            str(self.fixture.source / ".git"),
        ...
        mounts = [command[index + 1] for index, value in enumerate(command[:-1]) if value == "--mount"]
        self.assertIn(
            f"type=bind,src={(self.fixture.source / '.git').absolute()},dst=/input.git,readonly",
            mounts,
        )
        ...
        writable_source = output.parent.absolute()
        self.assertEqual(writable_source, self.fixture.output_dir.absolute())
        self.assertNotEqual(writable_source, self.fixture.root.absolute())
        for protected in (
            self.fixture.source,
            policy,
            self.fixture.home,
            self.fixture.xdg_config,
            self.fixture.global_config,
            self.fixture.template_dir,
            self.fixture.hooks_dir,
            self.fixture.root / "wheel-runtime",
        ):
            self.assertFalse(protected.absolute().is_relative_to(writable_source))
    ...
    def test_container_runner_keeps_safe_directories_in_global_configuration(self) -> None:
        ...
        self.assertEqual(
            fixture.global_config.read_text(),
            "[safe]\n\tdirectory = /input.git\n",
        )

    def test_host_runtimes_keep_global_configuration_empty(self) -> None:
        for runtime in ("source", "wheel"):
            with self.subTest(runtime=runtime), patch.dict(os.environ, {"GHS_TEST_RUNTIME": runtime}):
                fixture = GitFixture(self)
                self.assertEqual(fixture.global_config.read_text(), "")
    ...
    def test_container_verify_maps_source_and_output_repositories_separately(self) -> None:
        ...
        self.assertNotIn(f"type=bind,src={output.absolute()},dst=/input.git,readonly", output_mounts)
