...
class OutputContractTests(unittest.TestCase):
    ...
    def test_re_sanitizing_a_bare_output_is_deterministic_and_non_mutating(self) -> None:
        first_parent = self.fixture.output_dir / "first"
        first_parent.mkdir()
        first = first_parent / "sanitized.git"
        self.fixture.run_cli(
            "rewrite", "--source", str(self.fixture.source / ".git"), "--output", str(first),
            "--policy", str(self.policy)
        )
        first_snapshot = self.fixture.snapshot_output(first)
        second_parent = self.fixture.output_dir / "second"
        second_parent.mkdir()
        second = second_parent / "sanitized.git"
