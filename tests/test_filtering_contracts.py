...
class FilteringContractTests(unittest.TestCase):
    ...
    def test_empty_exclusions_preserve_allowed_tree_and_metadata(self) -> None:
        ...
        self.assertEqual(self.fixture.git(output, "show", "HEAD:allowed.txt"), "safe")
        self.assertEqual(self.fixture.git(output, "log", "-1", "--format=%an <%ae>"), "Fixture <fixture@example.invalid>")
        self.assertEqual(self.fixture.git(output, "log", "-1", "--format=%s"), "clean root")
    ...
    def test_exact_file_and_directory_exclusions_remove_reachable_history(self) -> None:
        ...
        objects = self.fixture.reachable_objects(output)
        self.assertEqual(names, ["keep.txt"])
        self.assertNotIn("secret.txt", objects)
        self.assertNotIn("private/key.txt", objects)
