...
This is the original project-specific prototype for running AI development
agents with restricted network, working-tree, and Git-history access. It is
kept alongside the standalone Git History Sanitize tool at the repository
root.

The example contains a small Express application plus its Docker
Compose/devcontainer sandbox. Run all commands below from this directory:
...
The Compose file reaches the enclosing repository's `.git` directory through
its `trusted_git` BuildKit context. If you copy this example into its own
repository, change that path from `../../.git` to `../.git`.
