...
### Commit consolidation follow-up
- [x] Rebuild the branch as a smaller set of meaningful commits
- [x] Verify the consolidated tree matches the published tree
- [x] Force-push the explicitly requested history rewrite

### Review-fix follow-up
- [x] Isolate the writable OCI output mount from all fixture inputs/configuration
- [x] Add source-only late-stage filter and verification failure contracts
- [x] Reproduce and resolve or evidence the macOS temporary-path warning
- [x] Track BBQ-7/8/11/12 consumer contracts after owner tickets land
- [x] Pass the complete local validation matrix and independent review
- [x] Commit incrementally, push, and confirm all PR checks pass

### CI repair follow-up
- [x] Diagnose OCI clone and local tamper failures
- [x] Add regression coverage and repair fixture configuration
- [x] Pass source discovery (54 tests) and OCI runtime contracts (28 tests)
- [x] Build wheel and source distribution; shell and whitespace checks
- [x] Evaluate and record clone/Colima learnings
- [x] Pass wheel runtime contracts (28 tests)
- [x] Commit fixes and learning documentation incrementally
- [x] Push fixes and confirm all PR checks pass

### OCI cleanup follow-up
- [x] Restore metadata-only input mounts and remove obsolete trust entries
- [x] Keep source/wheel global configuration empty
- [x] Correct superseded learnings and update regression tests
- [x] Validate source, wheel, OCI, build, and static checks
- [x] Independent cleanup review — PASS

Publication/remote gate: this cleanup is one commit. PR #2's checks on that
commit are the authoritative push/CI result, rather than predicting a remote
result in this pre-push document or adding a second documentation commit.
...
### 2026-09-06 — final commit consolidation

User explicitly requested another history consolidation and push, without a CI
wait. Rebuilding the branch into five coherent commits while preserving the
exact reviewed implementation tree. The five units cover pinned tooling, the
runtime harness, core contracts, CI/runtime integration, and documentation.
The pre-rewrite tree is retained locally at
`backup/BBQ-44-before-second-consolidation-fe192a7`. No new technical learning
arose from this mechanical history-only operation.

### 2026-09-06 — review fixes started

Evaluating four review findings against the current five-commit branch. The
writable output alias, late-stage failure coverage, macOS temporary environment,
and deferred owner-ticket obligations will be verified before implementation.
No owner-dependent BBQ-7/8/11/12 product semantics will be added here.

The writable-alias finding was confirmed: outputs beneath `fixture.root` made
the whole fixture writable at `/output`. Added `fixture.output_dir`, migrated
all outputs, and separated source/output parents for re-sanitization. Mount
construction coverage proves no protected path is beneath the writable source.
A live OCI probe confirmed `/input.git` is read-only and `/output/source/.git`
does not exist. Focused source tests pass (36 tests).

Added source-only `filter_paths` and post-rewrite `verify` failure injection.
Both contracts prove actionable CLI errors, source immutability, staging
cleanup, no final output publication, and redaction. The first run exposed an
unclosed policy stream via `ResourceWarning`; `Policy.from_file` now closes it
deterministically. Focused engine/policy tests pass (5 tests). Documented that
the macOS `DARWIN_USER_TEMP_DIR` warning is confined to the restricted
Codex/Seatbelt sandbox and is unaffected by fixture-owned temp variables.

Recorded explicit future consumer contracts in the relevant test modules for
BBQ-7 mixed-root messages, BBQ-8 independent cutoffCommit proof, BBQ-11
all-content-excluded behavior, and BBQ-12 complete plan/rewrite parity. The
owner tickets remain responsible for defining and implementing those semantics.

Final local validation passes: source discovery (57 tests), source/wheel/OCI
runtime contracts (28 each), wheel and sdist build, pinned toolchain validation,
shell syntax, and whitespace checks. No lint or typecheck command is configured.
Independent senior review returned PASS with no blocking or important findings.
Meaningful learnings from the review fixes are captured in
`docs/learnings/gotchas.md`; no additional learning arose from final validation.
Pushed through `31d818d`. CI run `34026522830` passes all four jobs: root
package tests, source/wheel/OCI contract matrix, prototype history fixture, and
Python distributions.

### 2026-09-06 — OCI cleanup

Removed whole-worktree mount special cases and consolidated input trust to
`/input.git`, only for container fixtures. Source/wheel global config is empty.
Updated mount and trust tests; corrected the misleading worktree requirement
and qualified the obsolete cross-owner trust guidance. Retained caller UID:GID,
missing-policy handling, Colima mount support, and inode-preserving tampering.
User requested a single cleanup commit. Source discovery passes 55 tests;
wheel and OCI contracts each pass 28. Wheel/sdist builds, shell syntax, and
whitespace checks pass. Independent review is PASS. No dedicated lint/typecheck
is configured. CI confirmation is tracked on the cleanup commit's PR checks.

### 2026-09-06 — CI repair verified

Pushed through `b8396ef`. CI run `34012771502` passes all four jobs: root
package tests, source/wheel/OCI contract matrix, prototype history fixture,
and Python distributions. Source discovery (54 tests), wheel contracts (28),
and OCI contracts (28) also passed locally during repair. No dedicated lint
or typecheck is configured; shell syntax and whitespace checks pass.
Learnings are captured above and in `docs/learnings/gotchas.md`.
House Rules remain satisfied with no isolation or verification exceptions.

### 2026-09-06

CI run 33984169246 reaches clone but reports only the final transport error.
Moved the narrow `/input` and `/output/*` safe-directory allowlist from
command-scope environment entries to the read-only fixture global config so
local upload-pack inherits it. No production code or isolation was relaxed.

Separately reproduced Colima virtiofs retaining a deleted config inode after
host Git replaces it. Remote tampering now appends in place; ref deletion uses
`update-ref -d` rather than branch deletion (which also rewrites config).
All 54 source tests and 28 OCI contracts pass, and distributions build.
No dedicated lint or typecheck is configured. Shell syntax and diff checks pass.
Wheel contracts also pass all 28 tests. Independent implementation review is
PASS. Remote CI confirmation remains pending.

Run 34011337212 still failed clone. A Linux tmpfs reproduction with UID 1001
source ownership and UID 0 execution exposed full stderr: upload-pack rejects
`/input/.git`, distinct from discovery's `/input`. Trusting both exact paths
in `/gitconfig` proves discovery, clone, rewrite, and verify succeed. Added the
missing exact entry and updated regression coverage; awaiting the next CI run.

Run 34012283295 passes clone/rewrite but exposes root-owned OCI outputs:
host-side tampering and cleanup fail with PermissionError. The fixture now
uses the caller's numeric UID:GID, keeping writable outputs caller-owned and
retaining all read-only mounts and network isolation. Added a command contract.
The fixed bare-source mount `/input.git` is also explicitly trusted for
Colima's ownership translation when re-sanitizing outputs. Linux tmpfs
UID-1001 validation passes rewrite, verify, tamper, and cleanup without chown.
...
- [x] Full pinned runtime image and source/wheel/OCI contract matrix
  (CI run `34012771502`; earlier bootstrap blockers resolved)
